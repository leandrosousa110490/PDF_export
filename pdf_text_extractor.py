    ...
    try:
        ...
        with open(pdf_path, 'rb') as file:
            ...
            for page_num in range(len(pdf_reader.pages)):
                ...
                # Method 2: Try extraction with different parameters
                try:
                    # Try extract_text with different parameters for better results
                    if hasattr(page, 'extract_text'):
                        # Try with space_width parameter for better word separation
                        alt_text = page.extract_text(space_width=200)
                        if alt_text and alt_text.strip() and alt_text != page_text_parts[0] if page_text_parts else True:
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for page_num, page in enumerate(pdf.pages):
                ...
                # ENHANCED Method 6: Extract text from all objects including non-selectable ones
                try:
                    # Try to extract text from all text objects on the page
                    objects = page.objects
                    for obj in objects.get('char', []):
                        if 'text' in obj and obj['text'].strip():
                            obj_text = obj['text']
                            if obj_text not in " ".join(page_text_parts):
                                page_text_parts.append(obj_text)
                except Exception as e:
                    print(f"   Object text extraction failed on page {page_num + 1}: {str(e)}")
                
                # ENHANCED Method 7: Try different extraction parameters for redacted content
                try:
                    # Extract with different layout parameters that might capture hidden text
                    for layout_params in [
                        {"x_tolerance": 1, "y_tolerance": 1},
                        {"x_tolerance": 5, "y_tolerance": 5},
                        {"x_tolerance": 10, "y_tolerance": 10}
                    ]:
                        try:
                            param_text = page.extract_text(**layout_params)
                            if param_text and param_text.strip():
                                param_clean = param_text.strip()
                                if param_clean not in " ".join(page_text_parts):
                                    page_text_parts.append(param_clean)
                        except Exception:
                            continue
                except Exception as e:
                    print(f"   Parameter-based extraction failed on page {page_num + 1}: {str(e)}")
                
                # ENHANCED Method 8: Extract from text lines with position information
                try:
                    text_lines = page.extract_text_lines()
                    for line in text_lines:
                        if 'text' in line and line['text'].strip():
                            line_text = line['text'].strip()
                            if line_text not in " ".join(page_text_parts):
                                page_text_parts.append(line_text)
                except Exception as e:
                    print(f"   Text lines extraction failed on page {page_num + 1}: {str(e)}")
    ...
    try:
        ...
        for page_num in range(len(doc)):
            ...
            try:
                ...
                # ENHANCED: Extract text from all text objects, including non-selectable ones
                try:
                    # Get text dictionary with detailed information
                    text_dict = page.get_text("dict", flags=11)  # Use flags for comprehensive extraction
                    for block in text_dict.get("blocks", []):
                        if block.get("type") == 0:  # Text block
                            for line in block.get("lines", []):
                                ...
                                for span in line.get("spans", []):
                                    ...
                                    if span_text:
                                        line_text += span_text
                                if line_text.strip() and line_text.strip() not in " ".join(page_text_parts):
                                    page_text_parts.append(line_text.strip())
                except Exception as e:
                    print(f"   Enhanced text dictionary extraction failed on page {page_num + 1}: {str(e)}")
                
                # ENHANCED: Try to extract text using textpage object for better redacted PDF handling
                try:
                    textpage = page.get_textpage()
                    if textpage:
                        # Extract all text from textpage
                        textpage_text = textpage.extractText()
                        if textpage_text and textpage_text.strip():
                            if textpage_text.strip() not in " ".join(page_text_parts):
                                page_text_parts.append(textpage_text.strip())
                        
                        # Try to extract text blocks from textpage
                        try:
                            blocks = textpage.extractBLOCKS()
                            for block in blocks:
                                if len(block) > 4 and block[4].strip():
                                    block_text = block[4].strip()
                                    if block_text not in " ".join(page_text_parts):
                                        page_text_parts.append(block_text)
                        except Exception:
                            pass
                except Exception as e:
                    print(f"   Textpage extraction failed on page {page_num + 1}: {str(e)}")
